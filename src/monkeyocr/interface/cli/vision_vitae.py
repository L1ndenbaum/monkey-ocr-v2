...
def main():
    ...
    image_processor = AutoImageProcessor.from_pretrained(
        model_name, trust_remote_code=True, max_pixels=1802240
    )
    ...
    image_inputs, _ = process_vision_info(
        messages, image_patch_size=image_processor.patch_size * image_processor.merge_size / 2
    )
    ...
    images_features = [
        rearrange(vision_embeddings, "(h w) c -> h w c ", h=grid_hw[0][0], w=grid_hw[0][1])
        for vision_embeddings, grid_hw in zip(vision_embeddings_all, grid_hw_all)
    ]
    print(
        "Input image size:",
        img_ori.size,
        "Extracted feature shapes:",
        [feature.shape for feature in images_features],
    )
