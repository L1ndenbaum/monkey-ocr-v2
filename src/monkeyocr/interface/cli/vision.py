...
def parse_args():
    parser = argparse.ArgumentParser(description="Extract image features with MonkeyOCRv2-S.")
    parser.add_argument(
        "--model-path",
        "-m",
        default="../model_weight/MonkeyOCRv2-B",
...
def main():
    ...
    image_processor = AutoImageProcessor.from_pretrained(
        model_path, trust_remote_code=True, max_pixels=1003520
    )
    ...
    images_feature = rearrange(
        vision_embeddings, "(h w m1 m2) c -> (h m1) (w m2) c ", h=h, w=w, m1=m, m2=m
    )

    print("Input image size:", img_ori.size, "Extracted feature shape:", images_feature.shape)
