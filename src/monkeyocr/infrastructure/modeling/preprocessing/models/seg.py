...
class sobel_net(nn.Module):
    def __init__(self):
        ...
        sobel_kernelx = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype="float32").reshape(
            (1, 1, 3, 3)
        )
        sobel_kernely = np.array([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype="float32").reshape(
            (1, 1, 3, 3)
        )
    ...
    def forward(self, im):  # input rgb
        x = (0.299 * im[:, 0, :, :] + 0.587 * im[:, 1, :, :] + 0.114 * im[:, 2, :, :]).unsqueeze(
            1
        )  # rgb2gray
