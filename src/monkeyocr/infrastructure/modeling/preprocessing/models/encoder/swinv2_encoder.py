...
class SwinV2Encoder(nn.Module):
    ...
    def load_pretrained_weights(self, pth_path):
        ...
        for model_key in model_dict.keys():
            if "relative_coords_table" in model_key or "relative_position_index" in model_key:
        ...
        missing_keys = [
            ele for ele in model_dict.keys() if not ele in loaded_keys and not ele in ignore_keys
        ]
        unexpected_keys = [
