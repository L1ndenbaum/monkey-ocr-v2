...
from monkeyocr.infrastructure.modeling.preprocessing.models.block import (
    build_lateral_connection,
    ConvWithActivation,
)
from monkeyocr.infrastructure.modeling.preprocessing.models.encoder.swin_transformer_v2 import (
    ...
    def __init__(
        ...
        if not self.skip_stages is None:
            self.lateral_connection_list = nn.ModuleList(
                [
                    build_lateral_connection(
                        int(encoder_dim * 0.5 ** (layer_idx + 1)), num_features[layer_idx]
                    )
    ...
    def forward(self, x, skip_features):
        ...
        for i in range(self.num_layers):
            ...
            if name in self.skip_stages:
                skip_feature = self.lateral_connection_list[self.skip_stages.index(name)](
                    skip_features[-i - 2]
                )
            ...
            if name in self.output_stages:
                ...
                x_out = norm_layer(x)
                x_out = (
                    x_out.view(-1, Wh, Ww, self.num_features[i]).permute(0, 3, 1, 2).contiguous()
                )
...
def load_pretrained_decoder(decoder, pth_path):
    ...
    for dec_key in decoder_dict.keys():
        if "relative_coords_table" in dec_key or "relative_position_index" in dec_key:
    ...
    missing_keys = [
        ele for ele in decoder_dict.keys() if not ele in loaded_keys and not ele in ignore_keys
    ]
    unexpected_keys = [
        ele for ele in pth_dict.keys() if not ele in loaded_keys and not ele in ignore_keys
    ]
